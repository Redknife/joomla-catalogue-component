...
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_catalogue&task=manufacturer.saveOrderAjax&tmpl=component';
...
<form action="<?php echo JRoute::_('index.php?option=com_catalogue&view=manufacturers'); ?>" method="post" name="adminForm" id="adminForm">
		...
		<table class="table table-striped" id="articleList">
			<thead>
				<tr>
					...
					<th>
						<?php echo JHtml::_('grid.sort', 'COM_CATALOGUE_HEADING_NAME', 'm.manufacturer_name', $listDirn, $listOrder); ?>
					</th>
					<th>
						<?php echo JHtml::_('grid.sort', 'COM_CATALOGUE_HEADING_COUNTRY', 'ct.country_name', $listDirn, $listOrder); ?>
					</th>
					<th width="1%" class="nowrap center hidden-phone">
					...
					<td class="nowrap has-context">
						<div class="pull-left">
							<?php if ($item->checked_out) : ?>
								<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'manufacturer.', $canCheckin); ?>
							<?php endif; ?>
							<?php if ($canEdit) : ?>
								<a href="<?php echo JRoute::_('index.php?option=com_catalogue&task=manufacturer.edit&id='.(int) $item->id); ?>">
						...
						<div class="pull-left">
							<?php
								...
								JHtml::_('dropdown.edit', $item->id, 'manufacturers.');
								JHtml::_('dropdown.divider');
								if ($item->state) :
									JHtml::_('dropdown.unpublish', 'cb' . $i, 'manufacturers.');
								else :
									JHtml::_('dropdown.publish', 'cb' . $i, 'manufacturers.');
								endif;

								JHtml::_('dropdown.divider');

								if ($archived) :
									JHtml::_('dropdown.unarchive', 'cb' . $i, 'manufacturers.');
								else :
									JHtml::_('dropdown.archive', 'cb' . $i, 'manufacturers.');
								endif;

								if ($item->checked_out) :
									JHtml::_('dropdown.checkin', 'cb' . $i, 'manufacturers.');
								endif;

								if ($trashed) :
									JHtml::_('dropdown.untrash', 'cb' . $i, 'manufacturers.');
								else :
									JHtml::_('dropdown.trash', 'cb' . $i, 'manufacturers.');
								endif;
