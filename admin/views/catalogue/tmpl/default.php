		...
		<table class="table table-striped" id="articleList">
			<thead>
				<tr>
					...
					<th>
						<?php echo JHtml::_('grid.sort', 'COM_CATALOGUE_HEADING_NAME', 'c.item_name', $listDirn, $listOrder); ?>
					</th>
					<th class="hidden-phone" width="30%">
						<?php echo JHtml::_('grid.sort', 'COM_CATALOGUE_HEADING_INTRO', 'c.description', $listDirn, $listOrder); ?>
					</th>
					<th>
					...
					<th width="1%" class="nowrap center hidden-phone">
						<?php echo JHtml::_('grid.sort', 'COM_CATALOGUE_HEADING_PRICE', 'c.price', $listDirn, $listOrder); ?>
					</th>
					<th width="1%" class="nowrap center hidden-phone">
			...
			<?php foreach ($this->items as $i => $item) :
				...
				<tr class="row<?php echo $i % 2; ?>" sortable-group-id="<?php echo $item->catid?>">
					...
					<td class="center">
						<?php echo JHtml::_('jgrid.published', $item->published, $i, 'catalogue.', $canChange, 'cb'); ?>
					</td>
					<td class="nowrap has-context" style="width: 25%;">
						<div class="pull-left">
							...
							<?php endif; ?>
							<div class="small">
								<?php echo $item->section_name .' > '. $item->category_name;?>
							</div>
						</div>
						<div class="pull-left">
							<?php
								...
								if ($item->published) :
									JHtml::_('dropdown.unpublish', 'cb' . $i, 'catalogue.');
								else :
									JHtml::_('dropdown.publish', 'cb' . $i, 'catalogue.');
								endif;

								JHtml::_('dropdown.divider');

								if ($archived) :
									JHtml::_('dropdown.unarchive', 'cb' . $i, 'catalogue.');
								else :
									JHtml::_('dropdown.archive', 'cb' . $i, 'catalogue.');
								endif;

								if ($trashed) :
									JHtml::_('dropdown.untrash', 'cb' . $i, 'catalogue.');
								else :
									JHtml::_('dropdown.trash', 'cb' . $i, 'catalogue.');
								endif;
