...
class CatalogueModelCatalogue extends JModelList
{
	...
	public function getCategories()
	{
		if (!isset($this->cache['categories'])) {
			...
			$query->from($db->quoteName('#__catalogue_category'));
			$db->setQuery($query);
			$this->cache['categories'] = $db->loadAssocList('id', 0);
		}
	...
	protected function getListQuery()
	{
		...
		$query->join('LEFT', '#__catalogue_category AS cat ON cat.id = c.category_id');
		
		$query->select('s.section_name');
		$query->join('LEFT', '#__catalogue_section AS s ON s.id = cat.section_id');
		
		$query->select('mf.manufacturer_name');
		$query->join('LEFT', '#__catalogue_manufacturer AS mf ON mf.id = c.manufacturer_id');
