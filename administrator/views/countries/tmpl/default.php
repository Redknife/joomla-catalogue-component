...
<form action="<?php echo JRoute::_('index.php?option=com_catalogue&view=countries'); ?>" method="post" name="adminForm" id="adminForm">
			...
			<thead>
				<tr>
					<th width="1%" class="nowrap center hidden-phone">
						<?php echo JHtml::_('grid.sort', '<i class="icon-menu-2"></i>', 'ctr.ordering', $listDirn, $listOrder, null, 'asc', 'JGRID_HEADING_ORDERING'); ?>
					</th>
					<th width="1%" class="hidden-phone">
						<input type="checkbox" name="checkall-toggle" value="" title="<?php echo JText::_('JGLOBAL_CHECK_ALL'); ?>" onclick="Joomla.checkAll(this)" />
					</th>
					<th width="1%" class="nowrap center">
						<?php echo JHtml::_('grid.sort', 'JSTATUS', 'ctr.published', $listDirn, $listOrder); ?>
					</th>
					<th>
						<?php echo JHtml::_('grid.sort', 'COM_CATALOGUE_HEADING_NAME', 'ctr.country_name', $listDirn, $listOrder); ?>
					</th>
					
					<th width="1%" class="nowrap center hidden-phone">
						<?php echo JHtml::_('grid.sort', 'JGRID_HEADING_ID', 'ctr.id', $listDirn, $listOrder); ?>
					</th>
				</tr>
			</thead>
			<tfoot>
					...
					<td class="nowrap has-context">
						<div class="pull-left">
							<?php if ($item->checked_out) : ?>
								<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'country.', $canCheckin); ?>
							<?php endif; ?>
							<?php if ($canEdit) : ?>
								<a href="<?php echo JRoute::_('index.php?option=com_catalogue&task=country.edit&id='.(int) $item->id); ?>">
						...
						<div class="pull-left">
							<?php
								...
								JHtml::_('dropdown.edit', $item->id, 'countries.');
								JHtml::_('dropdown.divider');
								if ($item->state) :
									JHtml::_('dropdown.unpublish', 'cb' . $i, 'countries.');
								else :
									JHtml::_('dropdown.publish', 'cb' . $i, 'countries.');
								endif;

								JHtml::_('dropdown.divider');

								if ($archived) :
									JHtml::_('dropdown.unarchive', 'cb' . $i, 'countries.');
								else :
									JHtml::_('dropdown.archive', 'cb' . $i, 'countries.');
								endif;

								if ($item->checked_out) :
									JHtml::_('dropdown.checkin', 'cb' . $i, 'countries.');
								endif;

								if ($trashed) :
									JHtml::_('dropdown.untrash', 'cb' . $i, 'countries.');
								else :
									JHtml::_('dropdown.trash', 'cb' . $i, 'countries.');
								endif;
